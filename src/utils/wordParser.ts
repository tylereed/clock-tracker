...
async function loadDefaultDictionary() {
  const dictionary = new Typo("en_US_FiveE_35", null, null, { dictionaryPath: "dictionaries", asyncLoad: true });
...
async function loadCachedDictionary(customWords: string[], loadDictionary: () => Promise<Typo>) {
...
export default async function formatParagraph(text: string, customWords: string[], loadDictionary: () => Promise<Typo> = loadDefaultDictionary) {
  const dictionary = await loadCachedDictionary(customWords, loadDictionary);
  // let log = "";
